...
const source = fs.readFileSync(path.join(__dirname, "../contracts/ClipVote.sol"), "utf8");

const input = {
  language: "Solidity",
  sources: { "ClipVote.sol": { content: source } },
  settings: {
...
const c = out.contracts["ClipVote.sol"]["ClipVote"];
...
fs.writeFileSync(path.join(__dirname, "../lib/clipvote_build.json"), JSON.stringify(build, null, 2));
console.log("compiler:", shortVersion);
console.log("bytecode length:", build.bytecode.length);
console.log("abi entries:", c.abi.length);
console.log("→ lib/clipvote_build.json");
